...
import { X, ImagePlus, Video, Trash2, ArrowUp, ArrowDown, Loader2, Plus } from "lucide-react";
...
export default function ProductFormModal({
  ...
  const removeImage = (index: number) => {
    ...
    // حذف فعلي من Cloudinary (لا يوقف الواجهة إن فشل، فقط تنبيه)
    api.deleteFile(removedUrl).catch(() => {
      toast.error("تم حذف الصورة من المنتج، لكن تعذّر حذفها نهائياً من التخزين");
    });
  };
  ...
  const addVariant = () => {
    ...
    if (!value) return;
    if (form.variants.includes(value)) {
      toast.error("هذا الخيار مضاف مسبقاً");
      return;
    }
    update("variants", [...form.variants, value]);
    setVariantInput("");
  };
      ...
      >
        ...
        >
          ...
          <form onSubmit={handleSubmit} className="p-5 space-y-4">
                ...
                {form.video ? (
                  <div className="relative rounded-xl overflow-hidden border border-brand-100 bg-brand-50 flex items-center gap-3 p-3">
                    ...
                    <button
                      type="button"
                      onClick={() => {
                        const removedUrl = form.video;
                        update("video", "");
                        api.deleteFile(removedUrl).catch(() => {
                          toast.error("تم حذف الفيديو من المنتج، لكن تعذّر حذفه نهائياً من التخزين");
                        });
                      }}
            ...
            {/* Variants: scents / colors */}
            <Field label="الروائح / الألوان المتوفرة (اختياري)">
              <div className="space-y-2">
                {form.variants.length > 0 && (
                  <div className="flex flex-wrap gap-2">
                    {form.variants.map((v, i) => (
                      <span
                        key={v + i}
                        className="flex items-center gap-1.5 pl-1 pr-3 h-8 rounded-full bg-brand-50 border border-brand-100 text-sm font-bold text-brand-purple-dark"
                      >
                        {v}
                        <button
                          type="button"
                          onClick={() => removeVariant(i)}
                          className="w-6 h-6 rounded-full flex items-center justify-center text-brand-pink hover:bg-brand-pink/10"
                          aria-label={`حذف ${v}`}
                        >
                          <Trash2 size={12} />
                        </button>
                ...
                <div className="flex gap-2">
                  <input
                    ...
                    placeholder="مثال: ورد، ياسمين، لافندر..."
                    className="input-field flex-1"
                  />
                  <button
                    type="button"
                    onClick={addVariant}
                    className="w-12 h-12 shrink-0 rounded-xl bg-brand-purple/10 text-brand-purple flex items-center justify-center hover:bg-brand-purple/20 transition-colors"
                ...
                <p className="text-xs text-brand-purple-dark/50">
                  اتركيها فارغة إن كان المنتج بدون خيارات. إذا أضفتِ خياراً واحداً على الأقل، ستُطالَب الزبونة باختيار واحد قبل الشراء.
                </p>
              </div>
            </Field>
